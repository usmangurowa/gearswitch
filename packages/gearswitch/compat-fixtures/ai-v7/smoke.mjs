// Compat smoke test for gearswitch + ai@7 (LanguageModelV4 / provider v4).
...
// hand-rolled LanguageModelV4 structural mock (shape mirrors
// packages/gearswitch/test/resilient-v4.test.ts).
function mockV4Model() {
  return {
    specificationVersion: 'v4',
    provider: 'compat-mock',
    modelId: 'compat-model-v7',
    supportedUrls: {},
    async doGenerate() {
      return {
        content: [{ type: 'text', text: 'hello from ai-v7' }],
...
const model = gearswitch({ models: [{ model: mockV4Model() }] });
const { text } = await generateText({ model, prompt: 'hi', maxRetries: 0 });
assert.equal(text, 'hello from ai-v7', 'generateText did not return mock text');
...
console.log('ai-v7 compat smoke: OK');
